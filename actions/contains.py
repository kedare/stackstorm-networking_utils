...
class Contains(Action):
    def run(self, item, container, item_type="address"):
        ...
        container_obj = ipaddress.ip_network(container)

        if item_type == "address":
            obj = ipaddress.ip_address(item)
            return obj in container_obj
        elif item_type == "network":
            obj = ipaddress.ip_network(item)
            return (container_obj.network_address <= obj.network_address and
